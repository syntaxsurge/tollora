...
- Autonomous agent templates, OpenAI planning and synthesis, deterministic
  ...
  retry retryable settlement and transient gateway/provider failures three times
  with fresh signed payloads before provider work is accepted. Refund recovery
  ...
  the current vault state can accept. Direct run reads refresh from Convex by
  run ID before using the in-memory run cache so polling clients see the latest
  ...
  transaction attempts render in expandable per-action diagnostics with attempt
  status, gas limit, retry delay, message, and transaction link when available.
  Long provider, gateway, and contract failure text stays inside expandable
  copyable diagnostics with user-facing summaries for budget, gas-floor,
